...
class AuctionInfoUtilComponent extends Component{
	...
	public function startup($controller){
		...
		$this->MGenre = $this->controller->MGenre;
		$this->AutoCallItem = $this->controller->AutoCallItem;
	...
	public function startup4shell(){
		...
		$this->AutoCallItem = new AutoCallItem();
		// murata.s ORANGE-537 ADD(E)
...
// 		unset($data['DemandInfo']['auction_deadline_time']);
	...
	public function update_auction_infos($demand_id, $data, $auto_selection = false){
		...
		//2017/07/11  ichino ORANGE-420 ADD end
			...
			// murata.s ORANGE-537 ADD(S)
			// オートコールまでの時間を取得
			$auto_call = $this->_get_auto_call_interval($data['DemandInfo']['priority']);

			// ジャンル別オートコールフラグの取得
			$auto_call_flag = $this->__get_auto_call_flag_m_genre($data['DemandInfo']['genre_id']);
			// murata.s ORANGE-537 ADD(E)
...
// 2017.01.04 murata.s ORANGE-244 ADD(E)
							...
							if(!empty($val['AffiliationAreaStat']['commission_unit_price_rank'])){
								...
								if(isset($rank_time[$rank])){
									...
									if(!empty($rank_time[$rank]) && $as){
										...
										// murata.s ORNAGE-537 ADD(S)
										if($auto_call != null && $this->_is_auto_callable($auto_call_flag, $val['MCorp']['auto_call_flag'])){
											$auction_data['AuctionInfo'][$i]['auto_call_time'] = date('Y-m-d H:i', strtotime($rank_time[$rank].'+'.$auto_call.'minute'));
											$auction_data['AuctionInfo'][$i]['auto_call_flg'] = 0;
										}else{
											$auction_data['AuctionInfo'][$i]['auto_call_time'] = null;
											$auction_data['AuctionInfo'][$i]['auto_call_flg'] = null;
										}
										// murata.s ORANGE-537 ADD(E)
										$i++;
									}
					...
					if(!$auto_selection){
						...
						if (0 < count($auction_data)) {
							...
							$this->__carry_auction_push_time($auction_data, $rank_time, $auto_call);
						...
						if (0 < count($auto_auction_data)) {
							...
							// murata.s ORANGE-537 CHG(S)
							$this->__carry_auction_push_time($auto_auction_data, $rank_time, $auto_call);
							// murata.s ORANGE-537 CHG(E)
						}
	 ...
	 * @param int $auto_call_interval オートコール時間
	 */
	private function __carry_auction_push_time(&$auction_data, $rank_time, $auto_call_interval){
				...
				if(!empty($highest_empty_rank)){
					foreach($auction_data['AuctionInfo'] as $key => $val){
						...
						if($val['rank'] == $rank['rank']){
							...
							if(!empty($auction_data['AuctionInfo'][$key]['auto_call_time'])){
								$auction_data['AuctionInfo'][$key]['auto_call_time'] = date('Y-m-d H:i', strtotime($rank_time[$highest_empty_rank].'+'.$auto_call_interval.'minute'));
							}
						}
						// murata.s ORANGE-537 CHG(E)
					}
...
// murata.s ORANGE-537 ADD(S)
	 ...
	 * @param int $category_id カテゴリID
	 */
	private function __get_auto_call_flag_m_genre($genre_id){

		$result = $this->MGenre->find('first', array(
				'conditions' => array('id' => $genre_id),
				'fields' => 'auto_call_flag'
		));

		return !empty($result) ? $result['MGenre']['auto_call_flag'] : 1;
	}

	/**
	 * ジャンル別オートコールフラグと企業別オートコールフラグからオートコールが可能かどうかチェックする
	 * @param number $genre ジャンル別オートコールフラグ
	 * @param number $corp 企業別オートコールフラグ
	 */
	private function _is_auto_callable($genre = 1, $corp = 1){
		if($genre != 1){
			// ジャンル別オートコールフラグがオートコール禁止であれば、オートコール対象外
			return false;
		}else if($corp != 1){
			// 企業別オートコールフラグがオートコール禁止であれば、オートコール対象外
			return false;
		}
	 ...
	 */
	private function _get_auto_call_interval($priority){
		$auto_call = $this->AutoCallItem->find('first');
		if($priority == Util::getDivValue('priority', 'asap')){
			$time = $auto_call['AutoCallItem']['asap'];
		}else if($priority == Util::getDivValue('priority', 'immediately')){
			$time = $auto_call['AutoCallItem']['immediately'];
		}else{
			$time = $auto_call['AutoCallItem']['normal'];
		}
		return $time;
	}
// murata.s ORANGE-537 ADD(E)
