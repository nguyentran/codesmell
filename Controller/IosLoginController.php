...
class IosLoginController extends AppController {
    public $name = 'IosLogin';
    ...
    public function login() {
        ...
        $this->log(array('post from iOS', $_POST), LOG_DEBUG);
	    ...
	    if ($deviceToken != "") {
            ...
            if ($res_fromDeviceToken) {
                ...
                if ($res_fromDeviceToken['DeviceInfo']['user_id'] == $user_id) {
                    ...
                    if ($result) {
                        $this->log(array('update push_cnt success', $set_data, $result), LOG_DEBUG);
                    } else {
                        $this->log(array('update push_cnt error', $set_data, $result), LOG_ERR);
                    }
                    ...
                    //user_idを後勝ちにする(update)、未読通知数を0にする
                    $set_data = array(
                        'user_id' => $user_id,
                        'push_cnt' => 0
                    );
                 ...
                 * エンドポイントがないので登録
                 */
                $res = $AmazonSnsUtil->createEndpoint($deviceToken, $this->_os_type);
